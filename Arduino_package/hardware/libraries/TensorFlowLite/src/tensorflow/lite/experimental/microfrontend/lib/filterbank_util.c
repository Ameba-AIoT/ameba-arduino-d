                            ...
                            int spectrum_size)
{
    ...
    if (state->channel_frequency_starts == NULL || state->channel_weight_starts == NULL || state->channel_widths == NULL || center_mel_freqs == NULL || actual_channel_starts == NULL || actual_channel_widths == NULL) {
    ...
    for (chan = 0; chan < num_channels_plus_1; ++chan) {
        ...
        if (width == 0) {
            ...
            const int padded_width =
                (((aligned_width - 1) / kFilterbankChannelBlockSize) + 1) * kFilterbankChannelBlockSize;
