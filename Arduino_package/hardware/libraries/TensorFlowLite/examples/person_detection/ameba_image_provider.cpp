...
#include "ameba_image_provider.h"

/*
 * The sample requires the following third-party libraries to be installed and
 * configured. Please follow the example guide on AmebaIoT official website:
...
TfLiteStatus InitCamera(tflite::ErrorReporter* error_reporter)
{
    ...
    // initialize SPI
    SPI.setDefaultFrequency(6000000);
    SPI.begin();
    // Reset the CPLD
                                   ...
                                   uint8_t* image_data)
{
    ...
    while (JpegDec.read()) {
        ...
        if (JpegDec.MCUx < skip_start_x_mcus || JpegDec.MCUx >= skip_end_x_mcu_index) {
            continue;
        }
