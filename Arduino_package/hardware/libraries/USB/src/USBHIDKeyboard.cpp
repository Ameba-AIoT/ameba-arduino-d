...
void USBHIDKeyboard::setReportID(uint8_t reportIDKeyboard, uint8_t reportIDConsumer)
{
    _reportIDKeyboard = reportIDKeyboard;
    _reportIDConsumer = reportIDConsumer;
}

void USBHIDKeyboard::consumerReport(uint16_t usage_code)
{
    _pHIDDev->inputReport(_reportIDConsumer, (uint8_t*)(&usage_code), sizeof(usage_code));
}

void USBHIDKeyboard::keyboardReport()
{
    _pHIDDev->inputReport(_reportIDKeyboard, (uint8_t*)(&report), sizeof(hid_keyboard_report_t));
}

void USBHIDKeyboard::keyboardReport(uint8_t modifiers, uint8_t keycode[6])
{
...
void USBHIDKeyboard::consumerPress(uint16_t usage_code)
{
    consumerReport(usage_code);
}

void USBHIDKeyboard::consumerRelease(void)
{
    consumerReport(0);
}

void USBHIDKeyboard::keyPress(uint16_t key)
{
    if (key == 0x00) {
        return;
    }
    ...
    if ((key >= HID_KEY_CONTROL_LEFT) && (key <= HID_KEY_GUI_RIGHT)) {
        ...
        if ((report.keycode[0] != key) && (report.keycode[1] != key) && (report.keycode[2] != key) && (report.keycode[3] != key) && (report.keycode[4] != key) && (report.keycode[5] != key)) {
...
void USBHIDKeyboard::keyRelease(uint16_t key)
{
    if (key == 0x00) {
        return;
    }
...
void USBHIDKeyboard::keyReleaseAll(void)
{
    memset(&report, 0, 8);
    keyboardReport();
}

void USBHIDKeyboard::keyCharPress(char ch)
{
...
void USBHIDKeyboard::keySequence(const char* str, uint16_t delayTime)
{
    char current, next;
    while (*str != '\0') {
        current = *str;
...
void USBHIDKeyboard::keySequence(String str, uint16_t delayTime)
{
