...
#if CONFIG_EXAMPLE_WLAN_FAST_CONNECT || (defined(CONFIG_INCLUDE_WPA_PSK))|| (defined(CONFIG_JD_SMART) && CONFIG_JD_SMART)
...
#endif
#if CONFIG_AUTO_RECONNECT
void (*p_wlan_autoreconnect_hdl)(rtw_security_t, char*, int, char*, int, int) = NULL;
#endif
...
void wifi_psk_info_set(struct psk_info * psk_data)
{
		...
		u32 param_buf[1];
		DCache_Clean(psk_data->psk_essid, (NDIS_802_11_LENGTH_SSID+4));
		DCache_Clean(psk_data->psk_passphrase, (IW_PASSPHRASE_MAX_SIZE + 1));
		DCache_Clean(psk_data->wpa_global_PSK, (A_SHA_DIGEST_LEN * 2));
		DCache_Clean(psk_data, sizeof(struct psk_info));
		param_buf[0] = psk_data;
...
void wifi_psk_info_get(struct psk_info * psk_data)
{
	...
	DCache_Invalidate(psk_data, sizeof(struct psk_info));
	DCache_Invalidate(psk_data->psk_essid, (NDIS_802_11_LENGTH_SSID+4));
	DCache_Invalidate(psk_data->psk_passphrase, (IW_PASSPHRASE_MAX_SIZE + 1));
	DCache_Invalidate(psk_data->wpa_global_PSK, (A_SHA_DIGEST_LEN * 2));
#endif
}
...
int wifi_get_associated_client_list(void * client_list_buffer, uint16_t buffer_length)
{
	...
	u32 param_buf[2];
	param_buf[0] = client_list_buffer;
	param_buf[1] = buffer_length;
	DCache_Clean(client_list_buffer, buffer_length);
	DBG_8195A("client buffer AP=%x\n",client_list_buffer);
	ret = inic_ipc_api_host_message_send(IPC_API_WIFI_GET_ASSOCIATED_CLIENT_LIST, param_buf, 2);
	DCache_Invalidate(client_list_buffer, buffer_length);
	return ret;
}
#if 0//INIC_IPC_API_TODO
//----------------------------------------------------------------------------//
...
int wifi_get_mode(const char *ifname, int * mode)
{
	...
	param_buf[0] = ifname;
	param_buf[1] = mode;
	DCache_Clean(ifname, strlen(ifname));
	DCache_Clean(mode, sizeof(int));

	ret = inic_ipc_api_host_message_send(IPC_API_WIFI_GET_MODE, param_buf, 2);
	DCache_Invalidate(mode, sizeof(int));
	return ret;
}
#if 0//INIC_IPC_API_TODO
...
int wifi_get_ssid(const char *ifname, unsigned char *ssid)
{
	...
	param_buf[1] = ssid;
	DCache_Clean(ifname, strlen(ifname));
	DCache_Clean(ssid, (32+1));//max ssid len

	ret = inic_ipc_api_host_message_send(IPC_API_WIFI_GET_SSID, param_buf, 2);
	DCache_Invalidate(ssid, 33);
	return ret;
}
#if 0//INIC_IPC_API_TODO
...
int wifi_get_setting(const char *ifname, rtw_wifi_setting_t *pSetting)
{
	...
	u32 param_buf[2];
	param_buf[0] = ifname;
	param_buf[1] = pSetting;
	DCache_Clean(ifname, strlen(ifname));
	DCache_Clean(pSetting, sizeof(rtw_wifi_setting_t));//may not needed??

	ret = inic_ipc_api_host_message_send(IPC_API_WIFI_GET_SETTING, param_buf, 2);
	DCache_Invalidate(pSetting, sizeof(rtw_wifi_setting_t));
	return ret;
}
//----------------------------------------------------------------------------//
...
void wifi_show_statistic(unsigned char idx)
{
	u32 param_buf[1];
	param_buf[0] = idx;
	inic_ipc_api_host_message_send(IPC_API_WIFI_SHOW_STATISTIC, param_buf, 1);
}
#if 0//INIC_IPC_API_TODO
...
int wifi_get_autoreconnect(__u8 *mode)
{
	...
	u32 param_buf[1];
#if CONFIG_AUTO_RECONNECT
	DCache_CleanInvalidate(mode, sizeof(__u8));
	param_buf[0] = mode;
