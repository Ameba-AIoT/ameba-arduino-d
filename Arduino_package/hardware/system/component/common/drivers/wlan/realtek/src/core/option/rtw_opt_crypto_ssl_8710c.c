...
#include "ssl/ssl_func_stubs/rom/rom_ssl_func_stubs.h"
...
int rtw_platform_set_calloc_free( void * (*calloc_func)( size_t, size_t ),
                              void (*free_func)( void * ) )
{
	__rom_stubs_ssl.init_rom_ssl_ram_map(calloc_func, free_func, NULL, rom_ssl_ram_map.use_hw_crypto_func);
}
/****************************************************************************************************
...
{
	int ret = 0;
	__rom_stubs_ssl.mbedtls_ecp_group_init(ecc);
	switch (group_id){
		case 19:
		      __rom_stubs_ssl.mbedtls_ecp_group_load( ecc, MBEDTLS_ECP_DP_SECP256R1);
			  __rom_stubs_ssl.mbedtls_mpi_init(&ecc->A);
			  __rom_stubs_ssl.mbedtls_mpi_copy(&ecc->A,&ecc->P);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,0,0);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,1,0);
			  break;
		case 20:
	 		  __rom_stubs_ssl.mbedtls_ecp_group_load( ecc, MBEDTLS_ECP_DP_SECP384R1);
			  __rom_stubs_ssl.mbedtls_mpi_init(&ecc->A);
			  __rom_stubs_ssl.mbedtls_mpi_copy(&ecc->A,&ecc->P);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,0,0);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,1,0);
			  break;
		case 21:
			  __rom_stubs_ssl.mbedtls_ecp_group_load( ecc, MBEDTLS_ECP_DP_SECP521R1);
			  __rom_stubs_ssl.mbedtls_mpi_init(&ecc->A);
			  __rom_stubs_ssl.mbedtls_mpi_copy(&ecc->A,&ecc->P);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,0,0);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,1,0);
			  break;
		case 25:
			  __rom_stubs_ssl.mbedtls_ecp_group_load( ecc, MBEDTLS_ECP_DP_SECP192R1);
			  __rom_stubs_ssl.mbedtls_mpi_init(&ecc->A);
			  __rom_stubs_ssl.mbedtls_mpi_copy(&ecc->A,&ecc->P);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,0,0);
			  __rom_stubs_ssl.mbedtls_mpi_set_bit(&ecc->A,1,0);
			  break;
		case 26:
			  __rom_stubs_ssl.mbedtls_ecp_group_load( ecc, MBEDTLS_ECP_DP_SECP224R1);
			  __rom_stubs_ssl.mbedtls_mpi_init(&ecc->A);
			  __rom_stubs_ssl.mbedtls_mpi_read_binary(&ecc->A,secp224r1_a,28);
			  break;
		default:
...
{
	__rom_stubs_ssl.mbedtls_mpi_free(&ecc->A);
	__rom_stubs_ssl.mbedtls_ecp_group_free(ecc);
}
...
int rtw_crypto_ecc_get_param_a(sae_ecc_crypto *ecc, sae_crypto_bignum *a)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_copy(a,&ecc->A));
...
int rtw_crypto_ecc_get_param_b(sae_ecc_crypto *ecc, sae_crypto_bignum *b)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_copy(b,&ecc->B));
...
int rtw_crypto_ecc_get_param_order(sae_ecc_crypto *ecc, sae_crypto_bignum *n)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_copy(n,&ecc->N));
...
int rtw_crypto_ecc_get_param_prime(sae_ecc_crypto *ecc, sae_crypto_bignum *prime)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_copy(prime,&ecc->P));
...
int rtw_crypto_ecc_point_read_bignum(sae_ecc_crypto *ecc,sae_crypto_bignum *x,sae_crypto_bignum *y,sae_ecc_point *p)
{
	...
	prime_len = __rom_stubs_ssl.mbedtls_mpi_size(&ecc->P);
	...
	ret = __rom_stubs_ssl.mbedtls_ecp_point_read_binary(ecc,p,buf,buf_len);
...
int rtw_crypto_ecc_point_write_bignum(sae_ecc_crypto *ecc,sae_crypto_bignum *x,sae_crypto_bignum *y,sae_ecc_point *p)
{
	...
	if(__rom_stubs_ssl.mbedtls_ecp_point_write_binary(ecc,p,MBEDTLS_ECP_PF_UNCOMPRESSED,&out_len,buf,buf_len) <0 ){
		ret = -1;
		goto cleanup;
	}
...
{
	__rom_stubs_ssl.mbedtls_ecp_point_init(point);
}
...
{
	__rom_stubs_ssl.mbedtls_ecp_point_free(point);
}
...
{
	return __rom_stubs_ssl.mbedtls_ecp_is_zero(point);
}
...
{
	int ret = 0;	
	MBEDTLS_MPI_CHK( __rom_stubs_ssl.mbedtls_ecp_mul( ecc, R, m, P, NULL, NULL ));
cleanup:
	if(ret < 0)
		ret = -1;
	return ret;
...
int rtw_crypto_ecc_point_is_on_curve(sae_ecc_crypto *ecc,sae_ecc_point *P)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_ecp_check_pubkey(ecc,P));
...
int rtw_crypto_ecc_point_add_point(sae_ecc_crypto *ecc,sae_ecc_point *X,sae_ecc_point *A,sae_ecc_point *B)
{
	...
	__rom_stubs_ssl.mbedtls_mpi_init(&one);
	
    MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_lset(&one,1));
    MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_ecp_muladd(ecc,X,&one,A,&one,B));

cleanup:
	if(ret < 0)
		ret = -1;
	__rom_stubs_ssl.mbedtls_mpi_free(&one);
	return ret;
}
...
{
	return __rom_stubs_ssl.mbedtls_ecp_point_cmp(P1,P2);
}
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_copy(X,Y));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_read_binary(X,buf,buf_len));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_write_binary(X,buf,buf_len));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_shift_r(X,count));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_add_mpi(X,A,B));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_add_int(X,A,b));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_sub_mpi(X,A,B));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_sub_int(X,A,b));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_div_mpi(Q,R,A,B));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_mod_mpi(X,A,N));
...
int rtw_crypto_bignum_add_mod(sae_crypto_bignum *X,sae_crypto_bignum *A,sae_crypto_bignum *B,sae_crypto_bignum *N)
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_add_mpi(X,A,B));
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_mod_mpi(X,X,N));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_mul_mpi(X,A,B));
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_mod_mpi(X,X,N));
...
{
	int ret = 0;
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_exp_mod(X,A,B,N,NULL));
...
int rtw_crypto_bignum_assign(sae_crypto_bignum *X, sae_crypto_bignum *Y,unsigned char inv)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_safe_cond_assign(X,Y,inv));
...
int rtw_crypto_bignum_inverse(sae_crypto_bignum *X, sae_crypto_bignum *A,sae_crypto_bignum *N)
{
	...
	MBEDTLS_MPI_CHK(__rom_stubs_ssl.mbedtls_mpi_inv_mod(X,A,N));
