...
#define UDP_TO_POWERSAVE_DEVICE	0
...
int udp_client_func(struct iperf_data_t iperf_data)
{
	...
	if (iperf_data.total_size == 0) {
		...
		while (((end_time - start_time) <= (configTICK_RATE_HZ * iperf_data.time)) && (!g_stream_id[iperf_data.stream_id].terminate)) {
			...
			memcpy(udp_client_buffer, &client_hdr, sizeof(client_hdr));
#if UDP_TO_POWERSAVE_DEVICE
			while ((skbbuf_used_num > (max_local_skb_num - 5) || skbdata_used_num > (max_skb_buf_num -6)))
#else
...
void cmd_iperf(int argc, char **argv)
{
	...
	if (stream_data->role == 's') {
		if (xTaskCreate(iperf_test_handler, "iperf_test_handler", BSD_STACK_SIZE, (void*) stream_data, tskIDLE_PRIORITY + 2 + PRIORITIE_OFFSET, &stream_data->task) != pdPASS) {
...
#endif
			if (xTaskCreate(iperf_test_handler, "iperf_test_handler", BSD_STACK_SIZE, (void*) stream_data_s, tskIDLE_PRIORITY + 2 + PRIORITIE_OFFSET, &stream_data_s->task) != pdPASS) {
				tptest_res_log("UDP ERROR: Create UDP server task failed.\n\r");
				goto exit;
			}
		}
		if (xTaskCreate(iperf_test_handler, "iperf_test_handler", BSD_STACK_SIZE, (void*) stream_data, tskIDLE_PRIORITY + 1 + PRIORITIE_OFFSET, &stream_data->task) != pdPASS) {
