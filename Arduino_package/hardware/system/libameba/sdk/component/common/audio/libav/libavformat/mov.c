...
#define MAX_BITS_INT64 20
void print64(int level,const char* note, uint64_t n){ 

    if(level > av_log_get_level())
		return;
    
    char buf[MAX_BITS_INT64] = {'0'};
    int pos = MAX_BITS_INT64;
    int i;
...
static int mov_read_covr(MOVContext *c, AVIOContext *pb, int type, int len)
{
    ...
    printf("###########SHOULD NEVER COME HERE(%s)!############\n", __func__);
...
static void mov_metadata_creation_time(AVDictionary **metadata, time_t time)
{
    ...
    if(time){
        ...
        current_tm.tm_mon += 1;
        sprintf(buffer, "%d-%d-%d %d:%d:%d", current_tm.tm_year, current_tm.tm_mon, current_tm.tm_mday, current_tm.tm_hour, current_tm.tm_min, current_tm.tm_sec);
...
static int mov_read_stco(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if(atom.type == MKTAG('s','t','c','o')){
        sc->chunk0_offset = avio_rb32(pb);
        for (i = 1; i < entries && !pb->eof_reached; i++)           
            avio_rb32(pb);
    }
    else if (atom.type == MKTAG('c','o','6','4')){
        sc->chunk0_offset = avio_rb64(pb);
        for (i = 1; i < entries && !pb->eof_reached; i++)
            avio_rb64(pb);
    }
    ...
    sc->chunk_count = i;
    av_log(c->fc, AV_LOG_INFO, "chunk_count %d\n",sc->chunk_count);
...
/*  Inorder to save sdram memory ,we shold,
    1, Do not use buf or a smaller buf to get sample_sizes 
    2, Set sample_sizes[i] from int to int16_t
    3, Not need to save IndexEntry table
*/
    sc->sample_sizes = av_malloc(entries * sizeof(int16_t));
    if (!sc->sample_sizes)
        return AVERROR(ENOMEM);

    for (i = 0; i < entries && !pb->eof_reached; i++) {
        sc->sample_sizes[i] = (int16_t)avio_rb(pb,field_size);
...
#endif
    sc->sample_count = i;
    av_log(c->fc, AV_LOG_INFO, "sample_count %d\n",sc->sample_count);
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
...
#if defined(CONFIG_PLATFORM_8195A) || defined(CONFIG_PLATFORM_8721D)
    sc->current_pos = sc->chunk0_offset;
#else
    ...
    av_freep(&sc->sample_sizes);   //we need sample_sizes when read sample
#endif
    av_freep(&sc->keyframes);
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        //print64(AV_LOG_DEBUG,"current_pos :",sc->current_pos);
 
        if(sc->current_sample >= sc->sample_count){
            av_log(s, AV_LOG_INFO, "read EOF\n");
            return AVERROR_EOF;
        }
        ...
        avio_seek(sc->pb, sc->current_pos, SEEK_SET);        
        ret = av_get_packet(sc->pb, pkt, sample_size);
        
        if (ret < 0)
            return ret;

        pkt->stream_index = sc->ffindex;
        pkt->pos = sc->current_pos;
