...
// disable prediction to reduce code size
#if 0
...
void PredCalcPrediction(float *act_spec, float *last_spec, int btype,
                        int nsfb,
                        int *isfb_width,
                        CoderInfo *coderInfo,
                        ...
                        int chanNum)
{
    ...
    float energy[BLOCK_LEN_LONG], snr_p[BLOCK_LEN_LONG], temp1, temp2;
    ChannelInfo *thisChannel;

    /* Set pointers for specified channel number */
    /* int psy_init; */
    int *psy_init;
    float (*dr)[BLOCK_LEN_LONG],(*e)[BLOCK_LEN_LONG];
    float (*K)[BLOCK_LEN_LONG], (*R)[BLOCK_LEN_LONG];
    float (*VAR)[BLOCK_LEN_LONG], (*KOR)[BLOCK_LEN_LONG];
    ...
    if (isRightWithCommonWindow) {
        ...
        for (i=0; i<nsfb; i++) {
            ...
            if(temp2<1.e-20)
                temp2=1.e-20;
            if(temp1!=0.0)
                snr[i]=-10.*log10((float ) temp2/temp1);
            else
                snr[i]=0.0;
