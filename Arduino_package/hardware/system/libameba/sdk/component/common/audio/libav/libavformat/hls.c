                          ...
                          struct variant *var, AVIOContext *in)
{
    ...
    while (!in->eof_reached) {
        ...
        if (av_strstart(line, "#EXT-X-STREAM-INF:", &ptr)) {
            ...
            if (var) {
            	var->finished = 1;
		// live streaming has no #EXT-X-ENDLIST, only if AOD
		av_log(NULL, AV_LOG_WARNING, "\r\n[Warning] rcvd #EXT-X-ENDLIST !!!\n");
            }
	...
	// if server dispatched segs out of order, revise "cur_seq_no" which is gonna play to the new
	// playlist's "start_seq_no" to prevent suspending play due to server's disordered behavior.
	if (var){
	if(var->cur_seq_no >= var->start_seq_no + var->n_segments) {
		av_log(NULL, AV_LOG_WARNING, "\r\n[Warning] OUT OF ORDER ! Revise cur_seq_no from %d to %d", var->cur_seq_no, var->start_seq_no);
