...
typedef struct
{
    ...
    /** device key */
    dev_key_p dev_key_list;
    uint16_t dev_key_num; //!< 15 valid bits
    /** virtual address */
    vir_addr_p vir_addr_list;
    ...
    uint8_t frnd_poll_times;
    uint8_t frnd_upd_times;
    /** beacon params */
    union
    {
...
/**
  * @brief clear all the mesh parameters
  *
  * It may reboot the system.
  * @return none
  */
...
/**
  * @brief get the node state from the nvm
  *
  * It can be usde to get the node state in advance.
  * @return node state
  */
...
/**
  * @brief get availabel app key index of the model
  * @param[in] pmodel_info: pointer of model info
  * @return the app key index
...
void mesh_model_bind_all_key(void);
...
uint8_t mesh_model_sub_dump(mesh_model_p pmodel, uint16_t addr[]);
void mesh_model_sub_load(mesh_model_p pmodel, uint16_t addr[], uint8_t num);
void mesh_model_sub_clear(mesh_model_p pmodel);
uint16_t mesh_model_bound_app_key_dump(mesh_model_p pmodel, uint16_t app_key_index_g[]);
/** @} */
