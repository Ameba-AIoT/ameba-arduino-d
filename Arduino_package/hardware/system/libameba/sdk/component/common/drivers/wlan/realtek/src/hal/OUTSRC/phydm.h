...
typedef struct _RX_High_Power_
{
	...
	#else
		#define ASSOCIATE_ENTRY_NUM				NUM_STA//8 // Max size of AsocEntry[].
	#endif
	#define	ODM_ASSOCIATE_ENTRY_NUM				ASSOCIATE_ENTRY_NUM
...
#define MAX_PATH_NUM_8195A		MAX_RF_PATH	// 1
#define MAX_PATH_NUM_8711B		1
#define MAX_PATH_NUM_8821C		1

//Max RF path
#if ((RTL8195A_SUPPORT == 1) || (RTL8711B_SUPPORT == 1) || (RTL8721D_SUPPORT == 1) )
#define ODM_RF_PATH_MAX MAX_PATH_NUM_8195A
#else
#define ODM_RF_PATH_MAX 2 //92c-series
#endif
#define ODM_RF_PATH_MAX_JAGUAR 4 //jaguar - series
...
#if ((RTL8711B_SUPPORT == 1) || (RTL8821C_SUPPORT == 1) || (RTL8721D_SUPPORT == 1))
...
typedef struct _ODM_Phy_Status_Info_
{
	...
	BOOLEAN		bBeamformed;				/* BF packet---*/
#endif
#endif // #if (RTL8195A_SUPPORT == 1)
#else // #if (DM_ODM_SUPPORT_TYPE &  (ODM_IOT))
	u1Byte		SignalQuality;	 		// in 0-100 index. 
	s1Byte		RxMIMOSignalQuality[4];	//per-path's EVM
	u1Byte		RxMIMOEVMdbm[4]; 		//per-path's EVM dbm
	u1Byte		RxMIMOSignalStrength[4];// in 0~100 index
...
//-------------REMOVED CASE-----------//
	...
	ODM_CMNINFO_EEPROMVERSION,
	ODM_CMNINFO_CRYSTALCAP,
	//-----------HOOK BEFORE REG INIT-----------//	
...
typedef struct _ODM_RA_Info_
{
	...
	// BIT1: EN_STBC
	// BIT2: LDPC_CAP
	// Add by Wilson 20130320
	// BIT3,BIT4 : 2SS short cut
	// BIT5 : init rate by rssi
	// BIT6: BF state
	// BIT7: delay try rate
...
typedef struct _FAST_ANTENNA_TRAINNING_
{
	...
	u4Byte	MainAnt_Sum_CCK[ODM_ASSOCIATE_ENTRY_NUM];
	u4Byte	AuxAnt_Sum_CCK[ODM_ASSOCIATE_ENTRY_NUM];
	u4Byte	MainAnt_Cnt_CCK[ODM_ASSOCIATE_ENTRY_NUM];
	u4Byte	AuxAnt_Cnt_CCK[ODM_ASSOCIATE_ENTRY_NUM];	
	u1Byte	RxIdleAnt;
...
#endif
{
	...
	u1Byte			current_rf_set_8821c;
	u1Byte			default_rf_set_8821c;
...
//------------CALL BY VALUE-------------//
	...
	u1Byte			BbSwingIdxCck[MAX_RF_PATH];
	u1Byte			BbSwingIdxCckCurrent;
	u1Byte			BbSwingIdxCckBase[MAX_RF_PATH];
	...
	s1Byte			Remnant_CCKSwingIdx[MAX_RF_PATH];
