...
void rtw_btcoex_DeInitialize(PADAPTER);
void rtw_btcoex_set_bt_state(PADAPTER padapter,u8 state);
...
#ifdef CONFIG_SW_MAILBOX_EN
sint	rtw_enqueue_mailbox(_queue *queue, struct mailbox_info *obj);
struct mailbox_info *rtw_dequeue_mailbox(_queue *queue);
u8 rtw_btcoex_send_mailbox(_adapter *padapter, u8 *data, u16 len);
#endif
#ifdef CONFIG_BT_COEXIST_SOCKET_TRX
