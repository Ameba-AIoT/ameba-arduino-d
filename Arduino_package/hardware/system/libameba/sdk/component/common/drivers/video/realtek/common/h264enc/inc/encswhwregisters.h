...
#define ASIC_INPUT_YUV420COMPRESSED     0x09
...
typedef enum {
    ...
    HEncLineBufferDepth,
    HEncLineBufferEn,
    HEncLineBufferLoopBackEn,
    HEncLineBufferHwIntfEn,
    HEncLineBufferIrqReset,
#endif
    HEncRefLumCompress,
    HEncRefChrCompress,
...
/* Encoder interrupt register */
    H1REG(HEncIRQRfcBufOverflow, 0x004, 0x00000800, 11, 1, RW, "IRQ RFC buffer overflow status bit."),
#ifdef RT_LINE_BUF
    H1REG(HEncIRQLineBuffer    , 0x004, 0x00000400, 10, 0, RO, "IRQ line buffer status bit. One line buffer reading has been done."),
#endif
...
/* External memory base addresses for encoder input/outputput */
    ...
    H1REG(HEncInputFormat      , 0x03c, 0x0000003c,  2, 1, RW, "Input image format. inputFormat. YUV420P/YUV420SP/YUYV422/UYVY422/RGB565/RGB555/RGB444/RGB888/RGB101010/YUV420Compressed"),
...
/* Mixed definitions JPEG / video */
    ...
    H1REG(HEncQpSum            , 0x064, 0x001fffff,  0, 0, RW, "QP Sum div2 output"),
...
#ifdef RT_LINE_BUF
    H1REG(HEncMbWrPtr             , 0x4b0, 0x000001ff,  0, 0, RW, "The number of MB line that is already filled in the line-buffer"),
    H1REG(HEncMbRdPtr             , 0x4b0, 0x0003fe00,  9, 0, RO, "The number of MB line that is already read out from the line-buffer"),
    H1REG(HEncLineBufferDepth     , 0x4b0, 0x07fc0000,  18, 1, RW, "Line Buffer Depth. unit is MB"),
    H1REG(HEncLineBufferEn        , 0x4b0, 0x08000000,  27, 1, RW, "Line Buffer Enable"),
    H1REG(HEncLineBufferLoopBackEn, 0x4b0, 0x10000000,  28, 1, RW, "Line Buffer Loop Back Enable. valid only when Sw_line_buffer_depth is smaller than frame size."),
    H1REG(HEncLineBufferHwIntfEn  , 0x4b0, 0x20000000,  29, 1, RW, "Line Buffer Hardware Interface Enable. Use hardware handshaking."),
    H1REG(HEncLineBufferIrqReset  , 0x4b0, 0x40000000,  30, 0, RW, "Reset Line Buffer IRQ when setting to 1"),
#endif
    /* Reference Frame Compression */
    H1REG(HEncRefLumCompress      , 0x4b4, 0x00000001,  0, 0, RW, "Reference Frame Luma Compression. 0=OFF. 1=ON"),
    H1REG(HEncRefChrCompress      , 0x4b4, 0x00000002,  1, 0, RW, "Reference Frame Chroma Compression. 0=OFF. 1=ON"),
    H1REG(HEncRfcLumDiscreteMode  , 0x4b4, 0x00000004,  2, 0, RW, "Reference Compression in discrete storage mode. 0=OFF. 1=ON"),
    H1REG(HEncRfcIntEn            , 0x4b4, 0x00000008,  3, 1, RW, "RFC overflow IRQ Enable. 0=Disable. 1=Enable"),
    H1REG(HEncRfcLumBufLimit      , 0x4b4, 0xfffff800,  11,1, RW, "Limit of luma RFC buffer in unit of 64-bit. 0 indicating no limit. Send an IRQ if overflow."),
    H1REG(HEncBaseRefLumTbl       , 0x4b8, 0xffffffff,  0, 0, RW, "Base address for reference luma compression table"),
    H1REG(HEncBaseRefChrTbl       , 0x4bc, 0xffffffff,  0, 0, RW, "Base address for reference chroma compression table"),
    H1REG(HEncBaseRecLumTbl       , 0x4c0, 0xffffffff,  0, 0, RW, "Base address for reconstructed luma compression table"),
    H1REG(HEncBaseRecChrTbl       , 0x4c4, 0xffffffff,  0, 0, RW, "Base address for reconstructed chroma compression table"),
    H1REG(HEncBaseRefLumTbl2      , 0x4c8, 0xffffffff,  0, 0, RW, "Base address for second reference luma compression table"),
    H1REG(HEncBaseRefChrTbl2      , 0x4cc, 0xffffffff,  0, 0, RW, "Base address for second reference chroma compression table"),

    /* for inputFormat = YUV420 Compressed */
    H1REG(HEncInputLeftOffset     , 0x4d0, 0x00003fff,  0, 0, RW, "Left offset of input luminance to the origin of input frame, only valid when input is compressed"),
    H1REG(HEncInputTopOffset      , 0x4d0, 0x0fffc000,  14,0, RW, "TOP offset of input luminance to the origin of input frame, only valid when input is compressed"),
    H1REG(HEncBaseInLumTbl        , 0x4d4, 0xffffffff,  0, 0, RW, "Base address for input luma compression table"),
    H1REG(HEncBaseInChrTbl        , 0x4d8, 0xffffffff,  0, 0, RW, "Base address for input chroma compression table"),
    H1REG(HEncBaseNextLumTbl      , 0x4dc, 0xffffffff,  0, 0, RW, "Base address for next pic luma compression table"),

    /* Reference Frame Compression */
    H1REG(HEncRfcChrBufLimit      , 0x4e0, 0xfffff000,  12,1, RW, "Limit of chroma RFC buffer in unit of 64-bit. 0 indicating no limit. Send an IRQ if overflow."),
};

#define ASIC_SWREG_AMOUNT               (0x4e0/4+1)
