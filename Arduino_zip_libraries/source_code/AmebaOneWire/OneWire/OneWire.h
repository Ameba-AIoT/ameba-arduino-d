...
#elif defined(BOARD_RTL8195A) || defined(BOARD_RTL8710) //for ameba 1
...
#elif defined(BOARD_RTL8721D)  // for ameba D
#define PIN_TO_BASEREG(pin)             ((uint32_t *)digitalPinToPort(pin))
#define PIN_TO_BITMASK(pin)             digitalPinToBitMask(pin)
#define IO_REG_TYPE                     uint32_t
#define IO_REG_ASM
...
#define DIRECT_WRITE_LOW(port, mask)    ( (*portOutputRegister((uint32_t)port)) &= ~mask )
#define DIRECT_WRITE_HIGH(port, mask)   ( (*portOutputRegister((uint32_t)port)) |=  mask )
#define DIRECT_MODE_INPUT(port, mask)   ( *( (volatile uint32_t *)( 0x48014004 + ((uint32_t)port) * 0x400 ) ) &= ~mask )
#define DIRECT_MODE_OUTPUT(port, mask)  ( *( (volatile uint32_t *)( 0x48014004 + ((uint32_t)port) * 0x400 ) ) |=  mask )
